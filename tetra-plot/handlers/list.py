...
async def callback_id_read(callback: CallbackQuery,
                           callback_data: PagesCallbackFactory,
                           connection: Connection):
    measurements_by = await db.measuring.get_measurements(connection, callback_data.series_id)
    measurements_list = []
    measurements_list.append(f"Вы выбрали серию измерений {callback_data.series_id} \n\n Сохранённые данные:\n")
    for measurement in measurements_by:
        measurements_list.append(f"{measurement.x} {measurement.y} {measurement.comment}")
